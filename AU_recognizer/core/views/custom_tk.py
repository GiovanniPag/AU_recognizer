...
class CustomTk(tk.Tk, CustomAppearanceModeBaseClass, CustomScalingBaseClass):
    """
    ...
    def __init__(self,
        ...
        tk.Tk.__init__(self, **pop_from_dict_by_set(kwargs, self._valid_tk_constructor_arguments))
        CustomAppearanceModeBaseClass.__init__(self)
        CustomScalingBaseClass.__init__(self, scaling_type="window")
        ...
        # indicates if the window is already shown through update() or mainloop() after init
        self._window_exists = False
        # indicates if withdraw() was called before window is first shown through update() or mainloop()
        self._withdraw_called_before_window_exists = False
        # indicates if iconify() was called before window is first shown through update() or mainloop()
    ...
    def _focus_in_event(self, _):
        # sometimes window looses jumps back on macOS if window is selected from Mission Control, so has to be lifted
        # again
        if sys.platform == "darwin":
            self.lift()

    def _update_dimensions_event(self, _=None):
        if not self._block_update_dimensions_event:
            ...
            if self._current_width != self._reverse_window_scaling(
                    detected_width) or self._current_height != self._reverse_window_scaling(detected_height):
                self._current_width = self._reverse_window_scaling(
                    detected_width)  # adjust current size according to new size given by event
                self._current_height = self._reverse_window_scaling(
                    detected_height)  # _current_width and _current_height are independent of the scale

    def _set_scaling(self, new_widget_scaling, new_window_scaling):
        super()._set_scaling(new_widget_scaling, new_window_scaling)
        # Force new dimensions on window by using min, max, and geometry. Without min, max it won't work.
        super().minsize(self._apply_window_scaling(self._current_width),
                        self._apply_window_scaling(self._current_height))
        super().maxsize(self._apply_window_scaling(self._current_width),
                        self._apply_window_scaling(self._current_height))
        super().geometry(
            f"{self._apply_window_scaling(self._current_width)}x{self._apply_window_scaling(self._current_height)}")
        # set new scaled min and max with delay (delay prevents weird bug where window dimensions snap to unscaled
        # dimensions when mouse releases window)
    ...
    def geometry(self, geometry_string: str = None):
        if geometry_string is not None:
            ...
            if width is not None and height is not None:
                self._current_width = max(self._min_width,
                                          min(width, self._max_width))  # bound value between min and max
    ...
    def _windows_set_titlebar_icon(self):
        try:
            ...
            if not self._iconbitmap_method_called:
                self.iconbitmap(asset("icons/au_icon.ico").as_posix())
        except Exception as err:
            logger.error(err)
            pass
    ...
    def _enable_macos_dark_title_bar(cls):
        if sys.platform == "darwin" and not cls._deactivate_macos_window_header_manipulation:  # macOS
            if version.parse(platform.python_version()) < version.parse("3.10"):
                if version.parse(tk.Tcl().call("info", "patchlevel")) >= version.parse("8.6.9"):  # Tcl/Tk >= 8.6.9
    ...
    def _windows_set_titlebar_color(self, color_mode: str):
        ...
        if sys.platform.startswith("win") and not self._deactivate_windows_window_header_manipulation:
            if self._window_exists:
                ...
                if (self._state_before_windows_set_titlebar_color != "iconic" or
                        self._state_before_windows_set_titlebar_color != "withdrawn"):
                    self.focused_widget_before_withdraw = self.focus_get()
                    # hide window so that it can be redrawn after the titlebar change so that the color change is
                    # visible
                    super().withdraw()
            else:
                self.focused_widget_before_withdraw = self.focus_get()
            ...
            if self.focused_widget_before_withdraw is not None:
                self.after(1, self.focused_widget_before_withdraw.focus)
                self.focused_widget_before_withdraw = None
