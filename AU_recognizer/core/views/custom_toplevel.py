...
class CustomToplevel(tkinter.Toplevel, CustomScalingBaseClass, CustomAppearanceModeBaseClass):
    """
    ...
    def __init__(self, *args,
        ...
        try:
            ...
            if sys.platform.startswith("win"):
                self.after(200, lambda: self.iconbitmap(asset("icons/au_icon.ico").as_posix()))
        ...
        # indicates if windows_set_titlebar_color was called, stays True until
        # revert_withdraw_after_windows_set_titlebar_color is called
        self._windows_set_titlebar_color_called = False
        # indicates if withdraw() was called after windows_set_titlebar_color
        self._withdraw_called_after_windows_set_titlebar_color = False
        # indicates if iconify() was called after windows_set_titlebar_color
    ...
    def _focus_in_event(self, _):
        # sometimes window looses jumps back on macOS if window is selected from Mission Control,
    ...
    def _windows_set_titlebar_color(self, color_mode: str):
        ...
        if sys.platform.startswith("win") and not self._deactivate_windows_window_header_manipulation:
            ...
            # hide window so that it can be redrawn after the titlebar change so that the color change is visible
            super().withdraw()
            super().update()
