...
from AU_recognizer.core.views import check_name, check_file_name, EntryButton, ComboEntry
...
class SelectFitImageDialog(Dialog):
    ...
    def create_view(self):
        ...
        fit_selected_button = ttk.Button(self.bottom_frame, text=i18n.dialog_buttons[I18N_FIT_SEL_BUTTON], command=self.fit_selected)
        fit_selected_button.pack(side=tk.RIGHT, padx=5, pady=5)
        fit_all_button = ttk.Button(self.bottom_frame, text=i18n.dialog_buttons[I18N_FIT_ALL_BUTTON], command=self.fit_all)
    ...
    def fit_all(self):
        ...
        emoca_fit(fit_data=self.fit_data, images_to_fit=Path(self.project[self._project_name][P_PATH]) / F_INPUT,project_data=self.project)
...
class SettingsDialog(Dialog):
    def __init__(self, master):
        ...
        self.main_frame = tk.Frame(self, padx=20, pady=20)
        self._i18n_path = EntryButton(master=self.main_frame, label_text="i18n_path",
                                      entry_text=nect_config[CONFIG][I18N_PATH])
        self._logger_path = EntryButton(master=self.main_frame, label_text="logger_path",
                                        entry_text=nect_config[CONFIG][LOGGER_PATH])
        self._log_folder = EntryButton(master=self.main_frame, label_text="log_folder",
                                       entry_text=nect_config[CONFIG][LOG_FOLDER])
        self._projects_folder = EntryButton(master=self.main_frame, label_text="projects_folder",
                                            entry_text=nect_config[CONFIG][PROJECTS_FOLDER])
        self._path_to_model = EntryButton(master=self.main_frame, label_text="path_to_model",
        ...
        self.languages_combobox = ComboEntry(master=self.main_frame, label_text="l_combo",
    ...
    def create_view(self):
        ...
        save_button = tk.Button(self.main_frame, text=i18n.dialog_buttons[I18N_SAVE_BUTTON], command=self.save_config)
        save_button.grid(row=6, column=0, sticky=tk.E, padx=10, pady=10)
        close_button = tk.Button(self.main_frame, text=i18n.dialog_buttons[I18N_CLOSE_BUTTON], command=self.close)
        close_button.grid(row=6, column=1, sticky=tk.EW, padx=10, pady=10)
    ...
    def save_config(self):
        ...
        changed = i18n.change_language(new_values[LANGUAGE])
        if changed:
