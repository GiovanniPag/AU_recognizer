...
class CustomScrollbar(CustomTKBaseClass):
    ...
    def __init__(self,
        ...
        # transfer basic functionality (_bg_color, size, __appearance_mode, scaling) to base class
        super().__init__(master=master, bg_color=bg_color, width=width, height=height, **kwargs)
        # color
        self._fg_color = ThemeManager.theme["CustomScrollbar"][
            "fg_color"] if fg_color is None else self._check_color_type(fg_color, transparency=True)
        self._button_color = ThemeManager.theme["CustomScrollbar"][
            "button_color"] if button_color is None else self._check_color_type(button_color)
        self._button_hover_color = ThemeManager.theme["CustomScrollbar"][
            "button_hover_color"] if button_hover_color is None else self._check_color_type(button_hover_color)
        # shape
        self._corner_radius = ThemeManager.theme["CustomScrollbar"][
            "corner_radius"] if corner_radius is None else corner_radius
        self._border_spacing = ThemeManager.theme["CustomScrollbar"][
    ...
    def _get_scrollbar_values_for_minimum_pixel_size(self):
        ...
        if self._orientation == "vertical":
            ...
            if (scrollbar_pixel_length < self._minimum_pixel_length
                    and -scrollbar_pixel_length + self._current_height != 0):
                # calculate how much to increase the float interval values so that the scrollbar width is
            ...
            if (scrollbar_pixel_length < self._minimum_pixel_length
                    and -scrollbar_pixel_length + self._current_width != 0):
                # calculate how much to increase the float interval values so that the scrollbar width is
                # self.minimum_pixel_length
                interval_extend_factor = (-scrollbar_pixel_length + self._minimum_pixel_length) / (
