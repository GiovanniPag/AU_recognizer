...
class DropdownMenu(tkinter.Menu, CustomAppearanceModeBaseClass, CustomScalingBaseClass):
    def __init__(self, *args,
        ...
        self._fg_color = ThemeManager.theme["DropdownMenu"]["fg_color"] if fg_color is None else self._check_color_type(
            fg_color)
        self._hover_color = ThemeManager.theme["DropdownMenu"][
            "hover_color"] if hover_color is None else self._check_color_type(hover_color)
        self._text_color = ThemeManager.theme["DropdownMenu"][
    ...
    def _check_font_type(font: any):
        if isinstance(font, CustomFont):
            ...
            logger.warn(
                f"Warning: font {font} given without size, will be extended with default text size of current theme\n")
            return font[0], ThemeManager.theme["text"]["size"]
        elif type(font) is tuple and 2 <= len(font) <= 3:
            return font
        else:
            raise ValueError(f"Wrong font type {type(font)} for font '{font}'\n" +
                             f"For consistency, Customtkinter requires the font argument to be a tuple of len 2 or 3 "
