...
class CustomAppearanceModeBaseClass:
    """
    ...
    def _check_color_type(color: any, transparency: bool = False):
        if color is None:
            ...
            return color
        elif isinstance(color, (tuple, list)) and len(color) == 2 and isinstance(color[0], str) and isinstance(color[1],
                                                                                                               str):
            return color
        else:
            raise ValueError(f"color {color} must be string ('transparent' or 'color-name' or 'hex-color') or tuple "
                             f"of two strings, not {type(color)}")
