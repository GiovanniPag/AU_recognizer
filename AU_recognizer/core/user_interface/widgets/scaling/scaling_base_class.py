...
class CustomScalingBaseClass:
    ...
    - destroy() must be called when subclass is destroyed
    ...
    def __init__(self, scaling_type: Literal["widget", "window"] = "widget"):
        ...
        if self.__scaling_type == "widget":
            ...
            self.__widget_scaling = ScalingTracker.get_widget_scaling()
            ...
            self.__window_scaling = ScalingTracker.get_window_scaling()
    ...
    def _apply_font_scaling(self, font: Union[Tuple, CustomFont]) -> tuple:
        """ Takes CustomFont object and returns tuple font with scaled size, has to be called again for every change
        of font object"""
        assert self.__scaling_type == "widget"
        ...
        elif isinstance(font, CustomFont):
            return font.create_scaled_tuple(self.__widget_scaling)
        else:
            raise ValueError(
                f"Can not scale font '{font}' of type {type(font)}. font needs to be tuple or instance of CTkFont")
