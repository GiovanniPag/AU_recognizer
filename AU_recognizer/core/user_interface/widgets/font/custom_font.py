...
class CustomFont(Font):
    ...
    over-strike	1 for over-struck text, 0 for normal.
    ...
    def __init__(self,
        ...
        self._size = ThemeManager.theme["CustomFont"]["size"] if size is None else size
        super().__init__(family=ThemeManager.theme["CustomFont"]["family"] if family is None else family,
                         size=-abs(self._size),
                         weight=ThemeManager.theme["CustomFont"]["weight"] if weight is None else weight,
                         slant=slant,
                         underline=underline,
    ...
    def config(self, *args, **kwargs):
        raise AttributeError("'config' is not implemented for Custom widgets. For consistency, always use 'configure' "
                             "instead.")
