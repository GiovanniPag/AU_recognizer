...
class CustomTextbox(CustomTKBaseClass):
    ...
    def __init__(self,
        ...
        # color
        self._fg_color = ThemeManager.theme["CustomTextbox"][
            "fg_color"] if fg_color is None else self._check_color_type(fg_color, transparency=True)
        self._border_color = ThemeManager.theme["CustomTextbox"][
            "border_color"] if border_color is None else self._check_color_type(border_color)
        self._text_color = ThemeManager.theme["CustomTextbox"][
            "text_color"] if text_color is None else self._check_color_type(text_color)
        self._scrollbar_button_color = ThemeManager.theme["CustomTextbox"][
            "scrollbar_button_color"] if scrollbar_button_color is None else self._check_color_type(
            scrollbar_button_color)
        self._scrollbar_button_hover_color = ThemeManager.theme["CustomTextbox"][
            "scrollbar_button_hover_color"] if scrollbar_button_hover_color is None else self._check_color_type(
            scrollbar_button_hover_color)

        # shape
        self._corner_radius = ThemeManager.theme["CustomTextbox"][
            "corner_radius"] if corner_radius is None else corner_radius
        self._border_width = ThemeManager.theme["CustomTextbox"][
            "border_width"] if border_width is None else border_width
        self._border_spacing = border_spacing

        # font
        self._font = CustomFont() if font is None else self._check_font_type(font)
        if isinstance(self._font, CustomFont):
            self._font.add_size_configure_callback(self._update_font)

        self._canvas = CustomCanvas(master=self,
        ...
        self._y_scrollbar = CustomScrollbar(self,
                                            width=8,
                                            height=0,
                                            border_spacing=0,
        ...
        self._x_scrollbar = CustomScrollbar(self,
                                            height=8,
                                            width=0,
                                            border_spacing=0,
                                            fg_color=self._fg_color,
        ...
        self._create_grid_for_text_and_scrollbars(re_grid_textbox=True, re_grid_x_scrollbar=True,
                                                  re_grid_y_scrollbar=True)

        self.after(50, self._check_if_scrollbars_needed, None, True)
        self._draw()

    def _create_grid_for_text_and_scrollbars(self, re_grid_textbox=False, re_grid_x_scrollbar=False,
                                             re_grid_y_scrollbar=False):

        # configure 2x2 grid
        self.grid_rowconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=0, minsize=self._apply_widget_scaling(
            max(self._corner_radius, self._border_width + self._border_spacing)))
        self.grid_columnconfigure(0, weight=1)
        self.grid_columnconfigure(1, weight=0, minsize=self._apply_widget_scaling(
            max(self._corner_radius, self._border_width + self._border_spacing)))

        if re_grid_textbox:
            self._textbox.grid(row=0, column=0, rowspan=1, columnspan=1, sticky="nsew",
                               padx=(self._apply_widget_scaling(
                                   max(self._corner_radius, self._border_width + self._border_spacing)), 0),
                               pady=(self._apply_widget_scaling(
                                   max(self._corner_radius, self._border_width + self._border_spacing)), 0))
        ...
        if re_grid_x_scrollbar:
            if not self._hide_x_scrollbar and self._scrollbars_activated:
                self._x_scrollbar.grid(row=1, column=0, rowspan=1, columnspan=1, sticky="ewn",
                                       ...
                                       padx=(max(self._corner_radius, self._border_width + self._border_spacing),
                                             0))  # scrollbar grid method without scaling
            else:
        ...
        if re_grid_y_scrollbar:
            if not self._hide_y_scrollbar and self._scrollbars_activated:
                self._y_scrollbar.grid(row=0, column=1, rowspan=1, columnspan=1, sticky="nsw",
                                       ...
                                       pady=(max(self._corner_radius, self._border_width + self._border_spacing),
                                             0))  # scrollbar grid method without scaling
            else:
                self._y_scrollbar.grid_forget()

    def _check_if_scrollbars_needed(self, _=None, continue_loop: bool = False):
    ...
    def configure(self, require_redraw=False, **kwargs):
        if "fg_color" in kwargs:
            ...
            # check if widgets are children of the frame and change their _bg_color to new frame fg_color
            for child in self.winfo_children():
                if isinstance(child, CustomTKBaseClass) and hasattr(child, "_fg_color"):
    ...
    def tag_add(self, tag_name, index1, index2=None):
        return self._textbox.tag_add(tag_name, index1, index2)

    def tag_bind(self, tag_name, sequence, func, add=None):
        return self._textbox.tag_bind(tag_name, sequence, func, add)

    def tag_cget(self, tag_name, option):
        return self._textbox.tag_cget(tag_name, option)
    ...
    def tag_delete(self, *tag_name):
        return self._textbox.tag_delete(*tag_name)

    def tag_lower(self, tag_name, below_this=None):
        return self._textbox.tag_lower(tag_name, below_this)

    def tag_names(self, index=None):
        return self._textbox.tag_names(index)

    def tag_nextrange(self, tag_name, index1, index2=None):
        return self._textbox.tag_nextrange(tag_name, index1, index2)

    def tag_prevrange(self, tag_name, index1, index2=None):
        return self._textbox.tag_prevrange(tag_name, index1, index2)

    def tag_raise(self, tag_name, above_this=None):
        return self._textbox.tag_raise(tag_name, above_this)

    def tag_ranges(self, tag_name):
        return self._textbox.tag_ranges(tag_name)

    def tag_remove(self, tag_name, index1, index2=None):
        return self._textbox.tag_remove(tag_name, index1, index2)

    def tag_unbind(self, tag_name, sequence, funcid=None):
        return self._textbox.tag_unbind(tag_name, sequence, funcid)
