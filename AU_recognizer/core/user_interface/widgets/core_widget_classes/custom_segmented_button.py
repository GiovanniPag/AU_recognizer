...
from . import CustomButton
from . import CustomFrame


class CustomSegmentedButton(CustomFrame):
    """
    ...
    def __init__(self,
        ...
        self._sb_fg_color = ThemeManager.theme["CustomSegmentedButton"][
            "fg_color"] if fg_color is None else self._check_color_type(fg_color)

        self._sb_selected_color = ThemeManager.theme["CustomSegmentedButton"][
            "selected_color"] if selected_color is None else self._check_color_type(selected_color)
        self._sb_selected_hover_color = ThemeManager.theme["CustomSegmentedButton"][
            "selected_hover_color"] if selected_hover_color is None else self._check_color_type(selected_hover_color)

        self._sb_unselected_color = ThemeManager.theme["CustomSegmentedButton"][
            "unselected_color"] if unselected_color is None else self._check_color_type(unselected_color)
        self._sb_unselected_hover_color = ThemeManager.theme["CustomSegmentedButton"][
            "unselected_hover_color"] if unselected_hover_color is None else self._check_color_type(
            unselected_hover_color)

        self._sb_text_color = ThemeManager.theme["CustomSegmentedButton"][
            "text_color"] if text_color is None else self._check_color_type(text_color)
        self._sb_text_color_disabled = ThemeManager.theme["CustomSegmentedButton"][
            "text_color_disabled"] if text_color_disabled is None else self._check_color_type(text_color_disabled)

        self._sb_corner_radius = ThemeManager.theme["CustomSegmentedButton"][
            "corner_radius"] if corner_radius is None else corner_radius
        self._sb_border_width = ThemeManager.theme["CustomSegmentedButton"][
            "border_width"] if border_width is None else border_width
        ...
        self._font = CustomFont() if font is None else font
        self._state = state

        self._buttons_dict: Dict[str, CustomButton] = {}  # mapped from value to button object
        if values is None:
            self._value_list: List[str] = ["CustomSegmentedButton"]
        else:
    ...
    def _get_index_by_value(self, value: str):
        ...
        raise ValueError(f"CustomSegmentedButton does not contain value '{value}'")
    ...
    def _configure_button_corners_for_index(self, index: int):
        if index == 0 and len(self._value_list) == 1:
            if self._background_corner_colors is None:
                self._buttons_dict[self._value_list[index]].configure(
            ...
            if self._background_corner_colors is None:
                self._buttons_dict[self._value_list[index]].configure(
                    background_corner_colors=(self._bg_color, self._sb_fg_color, self._sb_fg_color, self._bg_color))
            else:
                self._buttons_dict[self._value_list[index]].configure(background_corner_colors=(
                    self._background_corner_colors[0], self._sb_fg_color, self._sb_fg_color,
                    self._background_corner_colors[3]))

        elif index == len(self._value_list) - 1:
            if self._background_corner_colors is None:
                self._buttons_dict[self._value_list[index]].configure(
                    background_corner_colors=(self._sb_fg_color, self._bg_color, self._bg_color, self._sb_fg_color))
            else:
                self._buttons_dict[self._value_list[index]].configure(background_corner_colors=(
                    self._sb_fg_color, self._background_corner_colors[1], self._background_corner_colors[2],
                    self._sb_fg_color))

        else:
            self._buttons_dict[self._value_list[index]].configure(
                background_corner_colors=(self._sb_fg_color, self._sb_fg_color, self._sb_fg_color, self._sb_fg_color))
    ...
    def _create_button(self, _index: int, value: str) -> CustomButton:
        new_button = CustomButton(self,
                                  width=0,
        ...
        # DrawEngine rendering option (so that there's no gap between buttons)
        return new_button
    ...
    def _check_unique_values(values: List[str]):
        ...
        if len(values) != len(set(values)):
            raise ValueError("CustomSegmentedButton values are not unique")
    ...
    def insert(self, index: int, value: str):
        if value not in self._buttons_dict:
            if value != "":
                ...
                raise ValueError(f"CustomSegmentedButton can not insert value ''")
        else:
            raise ValueError(f"CustomSegmentedButton can not insert value '{value}', already part of the values")
    ...
    def move(self, new_index: int, value: str):
        if 0 <= new_index < len(self._value_list):
            if value in self._buttons_dict:
                ...
                raise ValueError(f"CustomSegmentedButton has no value named '{value}'")
        else:
            raise ValueError(
                f"CustomSegmentedButton new_index {new_index} "
                f"not in range of value list with len {len(self._value_list)}")
