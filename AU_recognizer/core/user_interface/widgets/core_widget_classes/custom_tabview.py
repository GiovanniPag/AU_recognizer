...
class CustomTabview(CustomTKBaseClass):
    ...
    def __init__(self,
        ...
        # transfer some functionality to CustomFrame
        super().__init__(master=master, bg_color=bg_color, width=width, height=height, **kwargs)

        # color
        self._border_color = ThemeManager.theme["CustomFrame"][
            "border_color"] if border_color is None else self._check_color_type(border_color)

        # determine fg_color of frame
        if fg_color is None:
            if isinstance(self.master, (CustomFrame, CustomTabview)):
                if self.master.cget("fg_color") == ThemeManager.theme["CustomFrame"]["fg_color"]:
                    self._fg_color = ThemeManager.theme["CustomFrame"]["top_fg_color"]
                else:
                    self._fg_color = ThemeManager.theme["CustomFrame"]["fg_color"]
            else:
                self._fg_color = ThemeManager.theme["CustomFrame"]["fg_color"]
        else:
        ...
        # shape
        self._corner_radius = ThemeManager.theme["CustomFrame"][
            "corner_radius"] if corner_radius is None else corner_radius
        self._border_width = ThemeManager.theme["CustomFrame"]["border_width"] if border_width is None else border_width
        self._anchor = anchor
        ...
        self._canvas = CustomCanvas(master=self,
                                    ...
                                    height=self._apply_widget_scaling(
                                        self._desired_height - self._outer_spacing - self._outer_button_overhang))
        ...
        self._tab_dict: Dict[str, CustomFrame] = {}
    ...
    def winfo_children(self) -> List[any]:
        """
        winfo_children of CustomTabview without canvas and segmented button widgets,
        because it's not a child but part of the CustomTabview itself
        """
    ...
    def _configure_segmented_button_background_corners(self):
        ...
        if self._fg_color == "transparent":
            ...
            if self._anchor.lower() in ("center", "w", "nw", "n", "ne", "e", "e"):
                self._segmented_button.configure(
                    background_corner_colors=(self._bg_color, self._bg_color, self._fg_color, self._fg_color))
            else:
                self._segmented_button.configure(
                    background_corner_colors=(self._fg_color, self._fg_color, self._bg_color, self._bg_color))
    ...
    def _configure_grid(self):
        ...
        if self._anchor.lower() in ("center", "w", "nw", "n", "ne", "e", "e"):
            ...
            self.grid_rowconfigure(2, weight=0, minsize=self._apply_widget_scaling(
    ...
    def _set_grid_segmented_button(self):
        ...
        if self._anchor.lower() in ("center", "n", "s"):
            self._segmented_button.grid(row=1, rowspan=2, column=0, columnspan=1,
                                        padx=self._apply_widget_scaling(self._corner_radius), sticky="ns")
        elif self._anchor.lower() in ("nw", "w", "sw"):
            self._segmented_button.grid(row=1, rowspan=2, column=0, columnspan=1,
                                        padx=self._apply_widget_scaling(self._corner_radius), sticky="nsw")
        elif self._anchor.lower() in ("ne", "e", "se"):
            self._segmented_button.grid(row=1, rowspan=2, column=0, columnspan=1,
                                        padx=self._apply_widget_scaling(self._corner_radius), sticky="nse")
        ...
        if self._anchor.lower() in ("center", "w", "nw", "n", "ne", "e", "e"):
            self._tab_dict[self._current_name].grid(row=3, column=0, sticky="nsew",
                                                    padx=self._apply_widget_scaling(
                                                        max(self._corner_radius, self._border_width)),
    ...
    def _create_tab(self) -> CustomFrame:
        new_tab = CustomFrame(self,
                              height=0,
                              width=0,
                              border_width=0,
                              corner_radius=0)
    ...
    def _draw(self, no_color_updates: bool = False):
        ...
        if no_color_updates is False or requires_recoloring:
            ...
            tkinter.Frame.configure(self, bg=self._apply_appearance_mode(
                self._bg_color))  # configure bg color of tkinter.Frame, cause canvas does not fill frame
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        if "segmented_button_unselected_hover_color" in kwargs:
            self._segmented_button.configure(
                unselected_hover_color=kwargs.pop("segmented_button_unselected_hover_color"))
    ...
    def tab(self, name: str) -> CustomFrame:
        """ returns reference to the tab with given name """

        if name in self._tab_dict:
            return self._tab_dict[name]
        else:
            raise ValueError(f"CustomTabview has no tab named '{name}'")

    def insert(self, index: int, name: str) -> CustomFrame:
        ...
        if name not in self._tab_dict:
            ...
            raise ValueError(f"CustomTabview already has tab named '{name}'")

    def add(self, name: str) -> CustomFrame:
    ...
    def move(self, new_index: int, name: str):
        if 0 <= new_index < len(self._name_list):
            if name in self._tab_dict:
                ...
                raise ValueError(f"CustomTabview has no name '{name}'")
        else:
            raise ValueError(
                f"CustomTabview new_index {new_index} not in range of name list with len {len(self._name_list)}")
