...
class CustomLabel(CustomTKBaseClass):
    """
    ...
    def __init__(self,
                 ...
                 anchor: Literal["nw", "n", "ne", "w", "center", "e", "sw", "s", "se"] = "center",  # label anchor
                 wraplength: int = 0,
                 **kwargs):
        # transfer basic functionality (_bg_color, size, __appearance_mode, scaling) to CTkBaseClass
        super().__init__(master=master, bg_color=bg_color, width=width, height=height)
        # color
        self._fg_color = ThemeManager.theme["CustomLabel"]["fg_color"] if fg_color is None else self._check_color_type(
            fg_color, transparency=True)
        self._text_color = ThemeManager.theme["CustomLabel"][
            "text_color"] if text_color is None else self._check_color_type(text_color)
        if text_color_disabled is None:
            if "text_color_disabled" in ThemeManager.theme["CustomLabel"]:
                self._text_color_disabled = ThemeManager.theme["CustomLabel"]["text_color"]
            else:
                self._text_color_disabled = self._text_color
        else:
            self._text_color_disabled = self._check_color_type(text_color_disabled)
        # shape
        self._corner_radius = ThemeManager.theme["CustomLabel"][
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        self._label.configure(
            **pop_from_dict_by_set(kwargs, self._valid_tk_label_attributes))  # configure tkinter.Label
    ...
    def cget(self, attribute_name: str) -> any:
        if attribute_name == "corner_radius":
            ...
            return super().cget(attribute_name)  # cget of super
