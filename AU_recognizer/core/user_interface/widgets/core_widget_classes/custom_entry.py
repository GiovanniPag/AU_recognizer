...
class CustomEntry(CustomTKBaseClass):
    ...
    def __init__(self,
        ...
        # transfer basic functionality (bg_color, size, appearance_mode, scaling) to CustomTKBaseClass
        ...
        # color
        self._fg_color = ThemeManager.theme["CustomEntry"]["fg_color"] if fg_color is None else self._check_color_type(
            fg_color, transparency=True)
        self._text_color = ThemeManager.theme["CustomEntry"][
            "text_color"] if text_color is None else self._check_color_type(text_color)
        self._placeholder_text_color = ThemeManager.theme["CustomEntry"][
            "placeholder_text_color"] if placeholder_text_color is None else self._check_color_type(
            placeholder_text_color)
        self._border_color = ThemeManager.theme["CustomEntry"][
            "border_color"] if border_color is None else self._check_color_type(border_color)

        # shape
        self._corner_radius = ThemeManager.theme["CustomEntry"][
            "corner_radius"] if corner_radius is None else corner_radius
        self._border_width = ThemeManager.theme["CustomEntry"]["border_width"] if border_width is None else border_width

        # text and state
        self._is_focused: bool = True
        self._placeholder_text = placeholder_text
        self._placeholder_text_active = False
        # some set arguments of the entry will be changed for placeholder and then set back
    ...
    def _create_grid(self):
        ...
        if self._corner_radius >= self._minimum_x_padding:
            self._entry.grid(column=0, row=0, sticky="nswe",
                             padx=min(self._apply_widget_scaling(self._corner_radius),
                                      round(self._apply_widget_scaling(self._current_height / 2))),
    ...
    def _textvariable_callback(self, _var_name, _index, _mode):
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        if "show" in kwargs:
            if self._placeholder_text_active:
                self._pre_placeholder_arguments["show"] = kwargs.pop(
                    "show")  # remember show argument for when placeholder gets deactivated
            else:
                self._entry.configure(show=kwargs.pop("show"))

        self._entry.configure(
            **pop_from_dict_by_set(kwargs, self._valid_tk_entry_attributes))  # configure Tkinter.Entry
        super().configure(require_redraw=require_redraw, **kwargs)  # configure CustomTkBaseClass
    ...
    def cget(self, attribute_name: str) -> any:
        if attribute_name == "corner_radius":
            ...
            return super().cget(attribute_name)  # cget of CustomTkBaseClass
    ...
    def _activate_placeholder(self):
        if self._entry.get() == "" and self._placeholder_text is not None and (
